...
%J = sum(y .* log(sigmoid(X*theta)) + (1-y) .* log(1 - sigmoid(X*theta))) * ( (-1)/m ) + lambda / ( 2 * m) * theta_temp' * theta_temp;
J = ( y' * log(sigmoid(X*theta)) + (1-y)' * log(1 - sigmoid(X*theta)) ) * ( (-1)/m ) + lambda / ( 2 * m) * theta_temp' * theta_temp;
